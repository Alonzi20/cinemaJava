...
public class CinemaSchedule extends Application {
  ...
  public void start(Stage primaryStage) {
    ...
    MovieSearchInterface searchInterface =
  ...
  private void visualizzaFilmFiltrati(List<Film> films) {
    ...
    if (films.isEmpty()) {
      ...
      container
          .getChildren()
          .removeIf(node -> node.getId() != null && node.getId().equals("nessunRisultatoLabel"));
  ...
  private HBox createMovieBox(Film movie) {
    ...
    ImageView poster =
        new ImageView(new Image("https://image.tmdb.org/t/p/w154" + movie.getPosterPath()));
